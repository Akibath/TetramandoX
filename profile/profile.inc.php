<?php
	...
	$userProfession = personProfession($profileID);

	$workStatus_LI = !empty($userProfession)? '<li>'.$userProfession[0].' at <span style="font:bold 14px helvetica;">'.$userProfession[1].'</span></li>': null;
	$currentLocation_LI = !empty($userLocation)? '<li>Lives at <span style="font:bold 14px helvetica;">'.$userLocation.'</span></li>': null;
	$birthDate_LI = '<li>Born on '.personAge($profileID, true).'</li>';
	$gender_LI = '<li>'.personGender($profileID, true).'</li>';
	
?>
<!DOCTYPE html>
<html>
<head>
	<title><?php echo personName($profileID, true, 30); ?> | TetramandoX</title>
		...
		<div class="profileContent">
			<div class="profilePanes">
				<div class="profilePane about">
					...
					<ul>
						<?php echo $workStatus_LI.$currentLocation_LI.$birthDate_LI.$gender_LI; ?>
					</ul>
				</div>
